...
class BaseTrainer(object):
    ...
    # Train an epoch
    def run_epoch(self, phase, epoch, data_loader, test_loader, opt2):
        """
        :param phase:
        :param epoch:
        :param data_loader:
        ...
        for batch_i, batch in enumerate(data_loader):
            ...
            for l in avg_loss_stats:
                ...
                # print(loss_stats[l], type(loss_stats[l]))
        ...
        if opt.add_test_dataset:
            opt_1 = opt2
            # TODO:add test iterations
            test_results = {}
            test_data_time, test_batch_time = AverageMeter(), AverageMeter()
            avg_test_loss_stats = {l: AverageMeter() for l in self.loss_stats}
            num_iters_test = len(test_loader) 
            bar_test = Bar('{}/{}'.format(opt_1.task, opt_1.exp_id), max=num_iters_test)
            end_test = time.time()
            
            with torch.no_grad():
                for batch_i, batch in enumerate(test_loader):
                    if batch_i >= num_iters_test:
                        break

                    test_data_time.update(time.time() - end_test)
                    ...
                    test_output, test_loss, test_loss_stats = model_with_loss(batch)

                    test_batch_time.update(time.time() - end)
                    end_test = time.time()

                    Bar.suffix = 'val: [{0}][{1}/{2}]|Tot: {total:} |ETA: {eta:} '.format(
                        epoch, batch_i, num_iters_test,
                        total=bar_test.elapsed_td, eta=bar_test.eta_td)
                    for l in avg_test_loss_stats:
                        
                        # print(loss_stats[l], type(loss_stats[l]))
                        avg_test_loss_stats[l].update(test_loss_stats[l].mean().item(), batch['input'].size(0))
                        Bar.suffix = Bar.suffix + '|{} {:.4f} '.format(l, avg_test_loss_stats[l].avg)

                    if opt_1.print_iter > 0:
                        if batch_i % opt_1.print_iter == 0:
                            print('{}/{}| {}'.format(opt_1.task, opt_1.exp_id, Bar.suffix))
                    else:
                        bar_test.next()
            bar_test.finish()
            
            ret_test = {k: v.avg for k, v in avg_test_loss_stats.items()}
            return ret, ret_test, results
        return ret, results
    ...
    def val(self, epoch, data_loader, test_loader, opt2):
        return self.run_epoch('val', epoch, data_loader, test_loader, opt2)

    def train(self, epoch, data_loader, test_loader, opt2):
        return self.run_epoch('train', epoch, data_loader, test_loader, opt2)
